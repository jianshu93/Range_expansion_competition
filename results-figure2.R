...
pdf(file = "Figure 2.pdf", width = 7, height = 8)

par(mfcol = c(2, 1), mar = c(5, 5, 2, 1), cex.axis = 1.1)
...
axis(2, at = seq(0, 350, 50), tcl = -.5, las = 1, pos = 0.7)
mtext(side = 3, expression(paste(bold("A"))), adj = -.13, cex = 1.3)
mtext(side = 3, expression(paste("Predicted expansion, No competition")), adj = 0, cex = 1.3)
for(i in 1:length(gens)){
    temp <- subset(stor.wetnocomp, Generation == gens[i])
    print(i)
    if(i < 10){
        points(Mean ~ Patch, data = temp, col = rgb(34 / 255, 139 / 255, 34 / 255, alpha = .5), type = "l", lwd = 2)
    } else{
        points(Mean ~ Patch, data = temp, col = rgb(34 / 255, 139 / 255, 34 / 255, alpha = 1), type = "l", lwd = 4)
    }
}
legend(x = 1, y = 370, adj = 0, lty = c("solid"), c(expression(italic("T. castaneum"))), bty = "n", pch = NA, cex = 1.2, col = c(rgb(34 / 255, 139 / 255, 34 / 255)))
...
axis(1, at = seq(20, 200, by = 20), tcl = -.5)
axis(2, at = seq(0, 350, 50), tcl = -.5, las = 1, pos = 0.7)
mtext(side = 3, expression(paste(bold("B"))), adj = -.13, cex = 1.3)
mtext(side = 3, expression(paste("Predicted expansion, Competition")), adj = 0, cex = 1.3)
for(i in 1:length(gens)){
...
legend(x = 1, y = 370, adj = 0, lty = c("solid", "dotted"), c(expression(italic("T. castaneum")), expression(italic("T. confusum"))), bty = "n", pch = NA, col = c(rgb(34 / 255, 139 / 255, 34 / 255), rgb(202 / 255, 44 / 255, 146 / 255)), cex = 1.2)
dev.off()
